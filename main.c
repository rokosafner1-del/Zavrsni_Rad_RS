...
//int ledToQuarter[10] = {0, 0, 0, 1, 1, 1,2, 2, 2, 3};
void PaliLed(int ledIndex, int trenKvadrant)
{
    ...
    // Ako je LED na PA0 (ledIndex == 0) pali je samo kad je kvartal 0
    if (ledIndex == 0 && trenKvadrant != 0) {
        return; // preskoči paljenje
    }

    // Inače upali traženu LED
...
{
    if (GPIO_Pin == GPIO_PIN_1)  // PC1 tipkalo
    {
    	if (!p){
    		p=1;
        ...
        //uint32_t trenutnoVrijeme = HAL_GetTick();

        // Debounce: tipka se neće registrirati dva puta unutar 300 ms
        //if (trenutnoVrijeme - zadnjeVrijemeTipke > 300)
        //{

           // zadnjeVrijemeTipke = trenutnoVrijeme;
        //}
   }
}
...
/*
  // Mapa LED-ica po kvartalima (rouletteNumbers[quarter][ledIndex])
...
*///>=
	    ...
	    for (int step = 0; step < steps; step++) {
	      ...
	      */
	      Click();
	      // kraći “click”
	      //PlayTone(3000, 15); // 3 kHz, 15 ms
	      // ili dulji zvuk
	      //PlayTone(440, 300); // 440 Hz, 300 ms (notna A4)
	      /*if (buzzerActive && HAL_GetTick() - buzzerStart >= 5) // 5 ms
	      {
	          __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, 0);
	          buzzerActive = 0;
	      }
...
/*
	    ...
	    // Ako je -1 (prazna pozicija u tablici) idi na index 1
	    if (rouletteNumbers[trenKvartal][finalLed] == -1) {
	        finalLed = 1;
	    }
	    //if (trenKvartal==0||finalLed==0 ) {
	    	//        trenLed = 0;
	    	//    }
...
*/
	   ...
	   // printf("Kvartal: %d  Led: %d\n", trenKvartal, trenLed);


	    // buffer za pretvorbu broja u tekst
...
/*
	    ...
	    sprintf(buf, "boja : %d", Rulet_GetColor(resultNumber));
	    LCD_Clear();
	    LCD_SendString(buf);
